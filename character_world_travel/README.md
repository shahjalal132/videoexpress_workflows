...
The agent asks for only three inputs:

1. Locations, in the desired order.
2. Total video length.
3. A reference image or a character idea.

It then determines the story structure, number of clips, exact per-clip durations, image prompts, time-coded video prompts, music theme, CloneVoice settings, and export name.
