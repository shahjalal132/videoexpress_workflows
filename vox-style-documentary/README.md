# VOX-Style Documentary Collage Video Workflow

Automated production of VOX-style documentary paper-collage animation videos using three browser tools:

| Tool | Role |
|---|---|
| **CloneVoice** (`app.clonevoice.ai`) | Narration audio (text-to-speech, "Tyler Brooks" voice by default) |
| **Artistly** (`app.artistly.ai`) | One editorial paper-collage image per beat (Fast AI Image Designer → Create From Prompt) |
| **VideoExpress** (`app.videoexpress.ai`) | Image-to-video clips (Create Video From Prompt), timeline assembly, export |

The full machine-readable contract — every URL, DOM selector, API endpoint, checkbox value, corner case, and the resume protocol — lives in **`vox_workflow.json`** (v1.4.0). This README is the human overview.
...
Phase 6  Import               - Artistly bridge (needs API key connected) or direct upload fallback
Phase 7  Clips                - Create Video From Prompt, batches of 5 (account cap), per-beat length,
                                acceptance verified by data.mediaId, both enhancers OFF, Video Only ON
