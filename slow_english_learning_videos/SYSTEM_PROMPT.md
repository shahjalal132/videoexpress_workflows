...
- Checkboxes by `name`: `auto_enhance_prompt` (default ON → uncheck), `use_consistent_character`, `talking_video` (Lipsync HD, leave off), `narration_video` (leave off), `video_only` (must be false), `shared` ("Share this in the public gallery", must be false), `advanced_mode` (turn ON → reveals `enhance_video_prompt` (keep false) and `manual_video_length` (turn ON) with range slider `#opt_video_duration` min 3 max 10).
- Setting the slider to an exact value N (changes per shot): read `min`, `max` and `step` of `#opt_video_duration` once. Click the slider thumb (the read-back rect of the input), press `End` (value becomes 10), then press `ArrowLeft` (10 − N) ÷ step times, and read `.value` live; it must equal N before Create Video. Dragging the thumb was verified only for reaching the maximum, so use the keyboard for intermediate values. A page-context `dispatchEvent` on the input is not allowed (UI changes must go through the control). If the read-back value differs from N, press `ArrowLeft`/`ArrowRight` until it matches, then re-read.
...
9. Shots are between 3 and 10 seconds because VideoExpress manual shot duration supports that range. Never default to 10 seconds; the duration of every shot is computed from its own dialogue (rule 11).
10. **DIALOGUE-FITTED CLIP DURATION — mandatory.** A clip is exactly as long as its spoken line needs, plus a short lead-in and reaction. Compute it in the prompt-book script, never by hand:
    - `actual_dialogue_words` = number of words inside the quotation marks (hyphenated words count as one; numbers count as one).
    - `speech_seconds = actual_dialogue_words / 1.5` (90 words per minute, the locked slow pace).
    - `duration_seconds = clamp(ceil(speech_seconds + 2), 3, 10)` — 1 second of lead-in before the first word and 1 second of reaction after the last word.
    - `dialogue_start_second = 1`, `dialogue_end_second = 1 + speech_seconds` (rounded to one decimal).
    - Examples: "Thank you, Uncle." → 3 words → 2.0 s speech → **4-second clip**. "Look, a big red truck!" → 5 words → 3.3 s → **6-second clip**. "We can buy one toy today, not three." → 8 words → 5.3 s → **8-second clip**. A 12-word line → 8.0 s → **10-second clip**.
...
24. Generate the prompt book with a script (for example `build_prompt_book.js`) so every lock sentence is byte-identical across shots and the duration of every clip is computed from its word count (rule 10), with the script throwing on any line over 12 words, any clip longer than its computed duration, and any total outside the runtime tolerance (rule 11). Keep the image prompt under ~3,000 characters and the video prompt under ~3,400 characters; both sizes were accepted by the form.
...
Verified cadence: one shot (keyframe + video submission) takes about 60–90 s of UI work and a video completes in about 60–120 s, so submitting shots strictly one after another never had more than two videos active at once. Do not wait for a video to finish before starting the next keyframe; do check the library API each turn and record completions.
...
5. Read the checkbox states: `auto_enhance_prompt=false`, `use_consistent_character=true`, `talking_video=false`, `narration_video=false`, `video_only=false`, `shared=false`, `advanced_mode=true`, `enhance_video_prompt=false`, `manual_video_length=true`; set `#opt_video_duration` to this shot's `duration_seconds` with the keyboard procedure in the UI map and read the live value back. The value changes from shot to shot; never leave a previous shot's value in place.
...
13. Run the dialogue timing gate. Count the words in the quoted line, recompute `duration_seconds = clamp(ceil(words / 1.5 + 2), 3, 10)`, and confirm it equals the prompt-book value and the slider value. A mismatch in any direction (line too long for the clip, or clip longer than the line needs) is a failure: fix the prompt book before submission.
...
16. Confirm `manual_video_length=true` and the live `#opt_video_duration` value equals this shot's `duration_seconds` (re-read it now; it was set in step 5 and must not be the previous shot's value).
...
3. Count active (`isPending: true`, `status: processing`) and completed (`status: completed`, `duration` ≈ the shot's `duration_seconds` × 1000 ms, e.g. 10041.667 for 10 s) deliverable jobs. A completed duration that does not match the shot's plan means the slider value was wrong at submission: record it as an error and regenerate that one shot with the correct value.
4. Update state.
