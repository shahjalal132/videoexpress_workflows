...
## PROMPT-BOOK MOUTH LOCK — HIGHEST-PRIORITY RULE

Every `final_videoexpress_prompt` must be **transformed into**, and pass, the mouth-locked best-practice architecture in §3 before any VideoExpress generation. Never reuse the old generic narration prefix, repetitive no-speech suffix, or `no mouth movement no lypsync` tail. A prompt that merely wraps raw Artistly text with negatives is invalid and must be rewritten before submission.
...
### Mouth-locked best-practice prompt architecture (STRICT — applies to every clip)

This section is the only valid method for writing `final_videoexpress_prompt`. It supersedes all historical prefix/suffix/tail wrappers. Never create a prompt by placing a generic no-speech paragraph before or after the raw Artistly text.

Use this canonical duration-independent template for every visible speaking-capable character:
...
Mandatory architecture and order:

1. shot continuity;
2. one brief mouth-set action at the very beginning;
3. the unchanged mouth/lower-face lock for the remainder of the clip;
4. character appearance and physical action;
5. emotion reassigned only to safe non-mouth channels;
6. environment, lighting, framing, and camera motion.

The first paragraph is a protected block. Preserve its meaning and strength in every scene. Do not move it to the end, abbreviate it to “mouth closed,” or split it around raw scene text.

Valid transformed example:
...
This example is valid because the mouth pose is established once, locked across connected lower-face anatomy, and never mentioned or contradicted later.

**Transform; never wrap.** Rewrite the Artistly Design Prompt into the architecture above. Preserve scene identity, wardrobe, physical action, environment, lighting, and camera intent, but remove any later mouth/facial-performance cue. After the protected first paragraph, the prompt must not contain another smile, grin, laugh, lip, mouth, teeth, jaw, cheek, chin, face-expression, speaking, singing, chanting, or mouthing instruction. Rewrite emotions into blinks, eye focus, eyebrow movement, head motion, hands, posture, clothing, and body movement.

Examples of required semantic rewrites:
...
- `animatedly`, `excited expression`, `look of discovery` → specify safe motion explicitly through eyes, eyebrows, head, hands, and posture;
- `open mouth`, `visible teeth`, `wide grin`, `big smile` → remove entirely; the protected mouth lock already defines the only allowed mouth pose.

Do not hard-code a clip duration inside the prose. Use “at the very beginning” and “for the remainder of the clip.” Advanced Mode controls the actual duration separately.

Do not freeze the whole face or body. Soft blinks, eye and eyebrow movement, slight head motion, hand gestures, posture, clothing, props, environmental motion, and camera movement remain active. The mouth shape must not change during blinks, head turns, body movement, jumping, spinning, or camera motion.

For multiple visible speaking-capable characters, apply the complete mouth-set and unchanged lower-face lock separately to each one. For non-human characters, name the relevant anatomy (muzzle, beak, mandible, or mouth seam) while retaining the equivalent sealed-mouth and still lower-face requirement.

Compact negative wording is sufficient. Never append the obsolete repetitive suffix or the misspelled `no mouth movement no lypsync` tail. Redundant negative lists dilute the positive pose and are forbidden.

Before storing each prompt, perform this binary checklist. Every item must pass:

- begins with `Single continuous` and contains no numeric prose duration;
- contains one beginning mouth-set action;
- contains `perfectly unchanged for the remainder of the clip`;
- explicitly locks lips, jaw, chin, cheeks, and lower face;
- excludes speaking, lip-sync, mouth opening, and visible teeth concisely;
- describes emotion only through safe non-mouth channels;
- contains no later smile, mouth, lower-face, vocal, or facial-expression performance cue;
- contains no generic wrapper, duplicated negative list, or `no mouth movement no lypsync` tail.

If any item fails, rewrite the prompt before it enters `prompt_book.json`. Never defer correction until VideoExpress.
...
8. Do not store the raw Artistly prompt in the prompt book. Store only the scene number, Design ID, image mapping, final VideoExpress prompt, and planned duration.
...
5. Blur or press Tab, then re-read the field once. Require exact equality with the prompt-book value and re-check the §3 mouth-locked architecture: protected opening present, unchanged lower-face lock present, and no obsolete wrapper or terminal tail.
...
- `prompt_book_gate`: `{path:"prompt_book.json", schema_version:"1.1.0", prompt_architecture:"mouth_locked_best_practice_v1", scene_count:N, unique_design_ids:true, unique_ve_image_ids:true, durations_total_s:ceil(audio_seconds), every_prompt_architecture_pass:true}`.
...
- `mouth_lock_gate`: `{source:"prompt_book.json", protected_opening:true, unchanged_lower_face_lock:true, later_contradiction_count:0, obsolete_wrapper:false, terminal_tail:false, image_prompt_enhancement_off:true, video_prompt_enhancement_off:true, video_only:true, advanced_mode:true, image_type:"3D", checked_before_submit:true}`.
...
11. **Characters act; they never speak.** Every prompt is transformed—not wrapped—into the §3 duration-independent architecture: mouth settles once, lips remain sealed, jaw/chin/cheeks/lower face stay perfectly unchanged, and emotion moves only through safe non-mouth channels. The obsolete repetitive suffix and `no mouth movement no lypsync` tail are forbidden. Enter the prompt-book value unchanged; keep both enhancement toggles off, Video Only and Advanced Mode on, and Image Type 3D.
