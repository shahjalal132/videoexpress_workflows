...
| [`WORKFLOW.json`](WORKFLOW.json) | Machine‑readable config (v2.10.0): strict mouth-locked prompt architecture, pre-authorized duplicate cleanup, verified audio transfer, structural-only storyboard acceptance, current VideoExpress flow, checkpoints, recovery, and safety rules. |
| [`PROMPT_GUIDE.md`](PROMPT_GUIDE.md) | **Binding prompt method.** The mouth-locked template, transformation steps, permitted variations, nine-item checklist, and a REJECTED reference showing the exact wrapper shape that is forbidden. |
...
| [`WORKFLOW_STATE.example.json`](WORKFLOW_STATE.example.json) | Compact v2.9.0 checkpoint example showing strict prompt architecture, pre-authorized cleanup, verified audio transfer, structural storyboard acceptance, latest-flow submission, and separate generation monitoring. |
| `README.md` | This file. |
...
- **Strict mouth-locked prompts — HARD RULE (v2.10.0)** — [`PROMPT_GUIDE.md`](PROMPT_GUIDE.md) is the single binding prompt method (also embedded in `SYSTEM_PROMPT.md` §3.1); no other style may be used, invented, or recalled from a previous run. Every scene carries a written nine-item `mouth_lock_check` in the prompt book, and a scene without a complete passing check cannot be submitted. Reusable `no_speech_constants`/prefix/suffix/tail objects are banned, and a prompt book below schema 1.1.0 or containing them is void and rebuilt. Every Artistly prompt is transformed, never generically wrapped. Each VideoExpress prompt begins as one continuous duration-independent shot, establishes a small closed-lip smile once, holds it perfectly unchanged, locks the lips/jaw/chin/cheeks/lower face, moves emotion only through safe non-mouth channels, and then describes environment/camera. Later smile or facial-expression cues, redundant negative suffixes, hard-coded prose durations, and the old **`no mouth movement no lypsync`** tail are forbidden. Prompt enhancement and lip sync remain off; completed media is still accepted without visual preview.
...
- **No duplicate-cleanup approval (v2.8.3)** — after ID mapping proves one extra unsaved timeline brick, the agent deletes only that duplicate, recounts to exactly `N`, checkpoints, and continues. It never asks for confirmation because timeline cleanup does not delete the generated/library video or a saved project.
